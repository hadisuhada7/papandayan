...
@section('title', 'Papandayan | Coverage Areas')
                        ...
                        <tbody>
                            ...
                            @foreach($areas as $area)
                                <tr>
                                    ...
                                    <td>{{ $area->partner_name }}</td>
                                    <td>{{ $area->area }}</td>
                                    <td>{{ $area->latitude }}</td>
                                    <td>{{ $area->longitude }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.areas.edit', $area) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $area->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.areas.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.areas.index') }}/" + selectedRow;
