...
@section('title', 'Papandayan | Document Reports')
                        ...
                        <tbody>
                            ...
                            @foreach($documents as $document)
                                <tr>
                                    ...
                                    <td>{{ $document->name }}</td>
                                    <td>
                                        <span class="badge {{ getJobStatusBadgeClass($document->status) }}">
                                            {{ getJobStatusDisplayText($document->status) }}
                                        </span>
                                    </td>
                                    <td><img src="{{ Storage::url($document->thumbnail) }}" alt="" style="max-width: 100px;"></td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.documents.edit', $document) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $document->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.documents.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.documents.index') }}/" + selectedRow;
