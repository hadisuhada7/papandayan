...
@section('title', 'Papandayan | Financial Statements')
                        ...
                        <tbody>
                            ...
                            @foreach($financials as $financial)
                                <tr>
                                    ...
                                    <td>{{ $financial->publish_at->format('d F Y') }}</td>
                                    <td>
                                        <span class="badge {{ getJobStatusBadgeClass($financial->status) }}">
                                            {{ getJobStatusDisplayText($financial->status) }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.financials.edit', $financial) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $financial->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.financials.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.financials.index') }}/" + selectedRow;
