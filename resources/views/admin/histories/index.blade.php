...
@section('title', 'Papandayan | Histories')
                    ...
                    <table id="datagrid" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 150px;">History At</th>
                        ...
                        <tbody>
                            ...
                            @foreach($histories as $history)
                                <tr>
                                    ...
                                    <td>{{ $history->name }}</td>
                                    <td>{{ $history->track_record_at->format('d F Y') }}</td>
                                    <td>{{ $history->description }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.histories.edit', $history) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $history->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.histories.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.histories.index') }}/" + selectedRow;
