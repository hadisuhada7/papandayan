...
@section('title', 'Papandayan | Investor Presentations')
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Investor Presentations</li>
            </ol>
        </div>
    </div>
@stop
                        ...
                        <tbody>
                            ...
                            @foreach($investors as $investor)
                                <tr>
                                    ...
                                    <td>{{ $investor->publish_at->format('d F Y') }}</td>
                                    <td>
                                        <span class="badge {{ getJobStatusBadgeClass($investor->status) }}">
                                            {{ getJobStatusDisplayText($investor->status) }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.investors.edit', $investor) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $investor->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.investors.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.investors.index') }}/" + selectedRow;
