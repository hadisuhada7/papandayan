...
@section('title', 'Papandayan | Stock Informations')
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Stock Informations</li>
            </ol>
        </div>
    </div>
@stop
                        ...
                        <tbody>
                            ...
                            @foreach($stocks as $stock)
                                <tr>
                                    ...
                                    <td>{{ $stock->title }}</td>
                                    <td>{{ $stock->publish_at->format('d F Y') }}</td>
                                    <td>
                                        <span class="badge {{ getJobStatusBadgeClass($stock->status) }}">
                                            {{ getJobStatusDisplayText($stock->status) }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.stocks.edit', $stock) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $stock->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                                @php 
                                    $index++; 
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.stocks.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.stocks.index') }}/" + selectedRow;
