...
@push('after-scripts')
    ...
    <script>
        $(function () {
            ...
            let selectedDocument = { id: null, name: '' };
            ...
            $('.item-download').on('click', function (e) {
                ...
                selectedDocument.id = $(this).data('id');
                selectedDocument.name = $(this).closest('.latestNewsCardInner').find('.latestNewsTxt h4 a').text().trim();

                $documentName.text(selectedDocument.name);
                resetFormState();
                $modal.modal('show');
            });
